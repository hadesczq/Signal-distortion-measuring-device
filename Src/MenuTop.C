...
{
    GUI_Clear();
    GUI_DispStringAtBar("Desktop",0,0,102,GUI_TA_HCENTER);
    GUI_DispStringAtBar("KC-ZG",0,16,102,GUI_TA_HCENTER);
    GUI_DispStringAtBar("Welcome!!!",0,32,102,GUI_TA_HCENTER);
    //显示按键对应的文字提示,0-->"菜单"
    GUI_DispStringAtBar(*(Tip[language].Text),0,Enter_PosY,102,GUI_TA_HCENTER);
}
//待机界面下Up键的处理
...
void spuare(uint8_t x1, uint8_t y1, uint8_t x2, uint8_t y2)
{
//  Dogs102x6_lineDraw(x1, y1-16, x1, y1, DOGS102x6_DRAW_NORMAL);
//  Dogs102x6_lineDraw(x1, y1, x1+98, y1, DOGS102x6_DRAW_NORMAL);
//  Dogs102x6_lineDraw(x1+98, y1-16, x1+98, y1, DOGS102x6_DRAW_NORMAL);
//  Dogs102x6_lineDraw(x1, y1-16, x1+98, y1-16, DOGS102x6_DRAW_NORMAL);
}
U8 zg_y = 0;
...
#ifdef LessCPUUseage
    if (HighLight == 1) { //高亮显示
        GUI_DispStringAtBar(s,X+8,Y,102,GUI_TA_LEFT);
...
//------------------------------------
     //   GUI_SetTextMode(GUI_TEXTMODE_REVERSE);//更改字体颜色来突出显示
        if(zg_y!=Y) Dogs102x6_lineDraw(0, zg_y+15, 98, zg_y+15, DOGS102x6_DRAW_INVERT);          
        GUI_DispStringAtBar(s,X,Y,102,GUI_TA_LEFT);
        Dogs102x6_lineDraw(0, Y+15, 98, Y+15, DOGS102x6_DRAW_NORMAL);  
        zg_y = Y;
...
//-------------------
    ...
    GUI_DispStringAtBar(*(DispItem+Item),0,ICO_PosY+ICO_YSize,102,GUI_TA_HCENTER);//菜单名字
    GUI_DrawIcon(icos[Item],ICO_PosX,ICO_PosY);             //图标

    //显示按键对应的文字提示,1-->"进入"
    GUI_DispStringAtBar(*(Tip[language].Text+1),0,Enter_PosY,102,GUI_TA_HCENTER);
