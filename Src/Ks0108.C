...
{
  Dogs102x6_charDraw(y0/8, x0, DisplayData, DOGS102x6_DRAW_NORMAL);
   //Dogs102x6_stringDrawXY(x0, y0, &DisplayData, 0);
  //ZG_102x6_setAddress(y0, x0);
  //ZG_Dogs102x6_writeData((uint8_t*)DisplayData, 6);  
...
//-------------------------------------------------------------------------
            ...
            for(i=0; i<32; i++){
                ...
                zg[i] = hzdot[i+CodeID*32];
               // Dogs102x6_stringDrawXY(HZ_X, y0, &ImgData, 0);
              //  Display_Locate(ImgData, HZ_X, y0);
                
              //  HZ_X++;
            }  
            ZG_102x6_Chinese(zg,y0/8,HZ_X);       
//-------------------------------------------------------------------------
             break;
...
//-------------------------------------------------------------------------
            ...
            for(i=16; i<32; i++){
              ImgData = hzdot[i+CodeID*32]^0xFF;
              Display_Locate(ImgData, HZ_X, y0 + 8);
              HZ_X++;
            }
//-------------------------------------------------------------------------
             break;
...
*****************************************************************************
*/
void Display_ASCII(U8 zg, U8 X, U8 Y)
{
  //U8 shu[1] = {0};
    Dogs102x6_charDraw(Y/8, X, zg, DOGS102x6_DRAW_NORMAL);
   // Dogs102x6_charDraw(Y/8, X, zg, DOGS102x6_DRAW_NORMAL);
   // Dogs102x6_charDraw(uint8_t row, uint8_t col, uint16_t f, uint8_t style)
//    Dogs102x6_stringDrawXY(X, Y, &zg, DOGS102x6_DRAW_NORMAL);
   // Dogs102x6_stringDraw(X,Y, &zg, DOGS102x6_DRAW_NORMAL);
  //Dogs102x6_stringDraw(X,Y, shu, DOGS102x6_DRAW_NORMAL);
/*    U8 i, ImgData;
    U8 HZ_X = X;
    if(GUIEnFont == En_5x8){
...
void GUI_DispCharAt(U8 c, U8 x, U8 y)
{
  uint8_t zg[1];
    ...
    zg[0] = (uint8_t)c;
    ZG_102x6_setAddress(y/8, x);
    ZG_Dogs102x6_writeData((uint8_t*)zg, 1);
  //Dogs102x6_charDraw(y/8, x, c, DOGS102x6_DRAW_NORMAL);
  //Dogs102x6_stringDrawXY(x, y, &c, 0);
    /*switch (GUITextMode) {
        case GUI_TEXTMODE_NORMAL :
             Display_ASCII(c, x, y);
             break;
...
{
    Dogs102x6_stringDrawXY(x0, y0, s, 0);
/*    U8 i;
...
//                PosY += 2;
            ...
            ZG_Display_Chinese(s[i]*256+s[i+1],PosX,PosY);//中文
            i++;
            PosX += 16;
       }
       i++;
...
*/
    XLen = ENCount*6 + CNCount*16;

    if (XLen > 102) {//我们只能显示一行
        XLen = 102;
    }
//识别对其方式属性
...
//-----------------------
    ...
    while(s[i]!='\0')
    {
        ...
        {//是中文
          //Display_ASCII(hzdot[0], 0, 10);
           // Display_Locate(hzdot[0], 0, 0);
           // Dogs102x6_stringDrawXY(0, 20, hzdot[0], 0);
           // while(1);
          //while(1);
    ...
    if(Mode&GUI_JUST_TEXT){
        ...
        if (GUITextMode == GUI_TEXTMODE_REVERSE){
            ...
            for(i = 0; i <x1-x0; i++){
             ...
             //   Display_Locate(0x00, x0+i,y0+2);
             //   Display_Locate(0x00, x0+i,y0+3);
            }
        }
        GUI_SetEnFont(En_5x8);
        return;
...
//    }
    ...
    for(i=0;i<32;i++){//第一行
        ZG_102x6_imageDraw(Ico[Count++],x0+i,y0);
    }
    for(i=0;i<32;i++){//第二行
        ZG_102x6_imageDraw(Ico[Count++],x0+i,y0+1);
    }
    for(i=0;i<32;i++){//第三行
        ZG_102x6_imageDraw(Ico[Count++],x0+i,y0+2);
    }
    for(i=0;i<32;i++){//第四行
        ZG_102x6_imageDraw(Ico[Count++],x0+i,y0+3);
    }
*/ 
}
#if 0
...
U8 _CONST_ BarCode0[]={0xFF,0xFf,0xFf,0xFf,0xFf,0xff,0xff,0xff,0xff};
//U8 _CONST_ BarCode1[]={0x00,0x01,0x03,0x07,0x0F,0x1F,0x3F,0x7F,0xFF};
U8 _CONST_ BarCode1[]={0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00};
...
//    Bool HighBit;
    ...
    ypot = BarLen*(Item_+1);
   // Temp = Item_*(BarEndPosY-BarBeginPosY);
   // Temp = Temp/(ItemNum_-1);
  //  YOffset = (U8)Temp;
/*    BarLen = (BarEndPosY+8-BarBeginPosY-BarRemainDot)/(ItemNum_);//BarLen根据ItemNum_得到
    if (BarLen == 0) {
        BarLen = 7;
    }
   // BarLen = 8;
    Temp = Item_*(BarEndPosY+8-BarBeginPosY-BarLen-BarRemainDot);//BarRemainDot是被扣除的部分
    Temp = Temp/(ItemNum_-1);
    YOffset = (U8)Temp;
    if(!Temp){//顶端，把预留的加上
...
*/    
 /*   for (Y = 0; Y < 6; Y++)
      {
        GUI_DispCharAt(0xFF,100,Y*8);//X=126列画线
        GUI_DispCharAt(0xFF,99,Y*8);//X=126列画线
        if(Y == 2) GUI_DispCharAt(0x00,99,Y*8);//X=126列画线
      }
    while(1);
 */  
    ZG_line(0xFF,BarPosX, BarEndPosY,(BarEndPosY-BarBeginPosY));
    ZG_line(0x00,BarPosX-1, BarEndPosY,(BarEndPosY-BarBeginPosY));
    ZG_line(0xff,BarPosX-1, ypot,BarLen);
 /*  for(Y = 0;Y < BarEndPosY/8;Y++)
    {
        if((Y != (YOffset/8))&&(Y != (YOffset/8+1)))
        {
            ZG_line(0x00,BarPosX,Y*8);//清除 X=125 列
        //    ZG_line(0xFF,BarPosX+1,Y*8);//X=126列画线
            ZG_line(0x00,BarPosX+2,Y*8);//清除 X=127 列
        }
        else
        {//Y = YOffset/8 Y = YOffset/8＋1
            ZG_line(BarCode0[YOffset%8],BarPosX,(YOffset/8)*8);
            ZG_line(0xFF-BarCode0[YOffset%8],BarPosX+1,(YOffset/8)*8);
            ZG_line(BarCode0[YOffset%8],BarPosX+2,(YOffset/8)*8);
            if((YOffset/8)+1 < (BarEndPosY/8))
            {//防止下越界
                ZG_line(BarCode1[YOffset%8],BarPosX,(YOffset/8+1)*8);
                ZG_line(0xFF-BarCode1[YOffset%8],BarPosX+1,(YOffset/8+1)*8);
                ZG_line(BarCode1[YOffset%8],BarPosX+2,(YOffset/8+1)*8);
...
*/
    ...
    if(U8_temp){
        Display_ASCII(U8_temp+'0',BarNumPosX-12,BarNumPosY);
        DispFlag = true;//通知低位显示
    }else{
        Display_ASCII(' ',BarNumPosX-12,BarNumPosY);
    }

    Item_ = (Item_-U8_temp*100);//剔除百位
    U8_temp = (U8)(Item_/10);//  十位
    if(U8_temp||(DispFlag == true)){//本位不为0，或者高位已经显示，那么必须显示
        Display_ASCII(U8_temp+'0',BarNumPosX-6,BarNumPosY);
        DispFlag = 1;
    }else{
        Display_ASCII(' ',BarNumPosX-6,BarNumPosY);
    }

    U8_temp = (U8)(Item_%10);//  个位
    Display_ASCII(U8_temp+'0',BarNumPosX+3,BarNumPosY);
...
//显示历史索引号
    if(ItemBackup_i > 1){//大于1才是
        for(i = 0; i <ItemBackup_i-1;i++){//最后一个位于1的位置
            ...
            U8_temp = (U8)(Item_%10);//
            Display_ASCII(U8_temp+'0',BarNumPosX-8*(i+1),BarNumPosY);
            
          //  ZG_102x6_setAddress(BarNumPosY/8, BarNumPosX-8*(i+1)+8-1);
          //  ZG_Dogs102x6_writeData((uint8_t*)c, 1);
            GUI_DispCharAt(0x08, BarNumPosX-8*(i+1)+8-1, BarNumPosY);//分隔符      
            GUI_DispCharAt(0x08, BarNumPosX-8*(i+1)+8, BarNumPosY);//分隔符    
        }
    }
//----------------------------------
    GUI_SetEnFont(En_8x16);
...
void ZG_line(char aa, char x, char y, char num)
{
  if(aa)  Dogs102x6_lineDraw(x, y-num+1, x, y, 0);
  if(!aa) Dogs102x6_lineDraw(x, BarBeginPosY, x, BarEndPosY, 1);
}
