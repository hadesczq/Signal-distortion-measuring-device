...
//       |  |  |  |  |       |
       ...
       {26, 0, 0, 0, 0,(*DispMenuMeasureUp)},   ////继续进入选项代码
...
/*    if(Key == Key_Up)
    {
      GUI_DispStringAt("Key_Up", 0, 10);
    }
    if(Key == Key_Down)
    {
      GUI_DispStringAt("Key_Down", 0, 20);
    }
    if(Key == Key_Left)
    {
      GUI_DispStringAt("Key_Left", 0, 30);
    }
    if(Key == Key_Right)
    {
      GUI_DispStringAt("Key_Right", 0, 40);
    }
    return 0;*/
    switch(Key){
